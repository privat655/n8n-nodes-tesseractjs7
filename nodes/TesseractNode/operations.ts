...
export const HARD_MAX_PDF_SIZE_MB = 50;
...
async function withTimeout<T>(promise: Promise<T>, timeout: number): Promise<T | TimeoutResult> {
	if (!Number.isFinite(timeout) || timeout <= 0) {
		return promise;
	}

	return new Promise<T | TimeoutResult>((resolve, reject) => {
		let settled = false;
		...
		const timer = setTimeout(() => {
			...
			// Mark the timeout as settled before resolving. This prevents a
			// concurrently finishing OCR promise from winning after timeout.
			settled = true;
			resolve(TIMEOUT);
		}, timeout);
...
async function terminateTimedOutWorker(
	...
	try {
		...
		this.logger.warn('Failed to terminate Tesseract worker after timeout', {
			imageName,
			error,
		});
	}
...
async function processPDFImage(
	this: IExecuteFunctions,
	doc: PDFDocument,
	ref: PDFRef,
	obj: PDFRawStream,
): Promise<ImageWithName | undefined> {
	const { dict } = obj;
	...
	const image: Partial<ImageWithName> = {
		name: name?.asString() ?? `Object${ref}`,
		ref: ref,
		maskRef: smaskRef,
	};

	this.logger.debug('reading image', {
		name: image.name,
		ref,
		filter,
		colorSpace,
		...
		case PDFName.of('FlateDecode'): // probably raw image data in one of many, many shapes
			...
			switch (true) {
				case colorSpace === PDFName.of('DeviceGray'): // 1-bit (binary) or 8-bit (grayscale) images
					...
					if (bitsPerComponent === 1) {
						...
						for (let i = 0; i < rawPixels.length; i++) {
							// each i grabs a bunch of 8 pixels
							for (let j = 0; j < 8; j++) {
								// j chooses each pixel in the bunch
				...
				case colorSpace instanceof PDFRef: // indexed images (colors live in a separate "palette", pixel values are indices into said palette)
					...
					if (
						colorSpaceObject.get(0) !== PDFName.of('Indexed') ||
						colorSpaceObject.get(1) !== PDFName.of('DeviceRGB')
					) {
					...
					const paletteRaw: Uint8Array = inflate(
						(doc.context.lookup(colorSpaceObject.get(3)) as PDFRawStream).contents,
					);
					...
					jimpImage.scan((x: number, y: number) => {
						// fill the image with the correct colors from the palette
						const idx = x + y * width;
			...
			if (smaskRef) {
				const jimpImageMask = await Jimp.fromBuffer(
					(await processPDFImage.call(
						this,
						doc,
						smaskRef,
						doc.context.lookup(smaskRef) as PDFRawStream,
					))!.data,
				);
				// @ts-ignore
			...
			return image as ImageWithName;
		default:
			this.logger.warn('unhandled image config', {
				ref,
				name,
				filter,
				colorSpace,
...
async function getImagesFromBinary(
	this: IExecuteFunctions,
	itemIndex: number,
	imageFieldName: string,
	resizeFactor: number = 100,
	maxPdfSizeMb: number = DEFAULT_MAX_PDF_SIZE_MB,
): Promise<ImageWithName[]> {
	...
	if (binaryInfo.mimeType.startsWith('image/')) {
		...
		if (resizeFactor !== 100) {
			resized = await (await Jimp.fromBuffer(buffer))
				.scale(resizeFactor / 100)
				.getBuffer('image/jpeg');
			mimeType = 'image/jpeg';
		}
		return [{ data: resized, mimetype: mimeType, name: binaryInfo.fileName! }];
	} else if (binaryInfo.mimeType === 'application/pdf') {
		const maxPdfSizeBytes = maxPdfSizeMb * BYTES_PER_MB;

		if (buffer.length > maxPdfSizeBytes) {
			const actualSizeMb = buffer.length / BYTES_PER_MB;
			...
			throw new NodeOperationError(
				...
				{},
				{
					itemIndex,
					message:
						`PDF binary property "${imageFieldName}" is ` +
						`${actualSizeMb.toFixed(2)} MB and exceeds the configured ` +
						`maximum of ${maxPdfSizeMb} MB`,
				},
			);
		}
		...
		pdfDoc.context.enumerateIndirectObjects().forEach(([pdfRef, pdfObject], ref) => {
			if (
				!(pdfObject instanceof PDFRawStream) ||
				pdfObject.dict.get(PDFName.of('Subtype')) !== PDFName.of('Image')
			) {
				return;
			}
		...
		const maskRefs = new Set<PDFRef>(
			allImages.filter((img) => img.maskRef !== undefined).map((img) => img.maskRef!),
		);
...
export async function performOCR(
	...
	const images = await getImagesFromBinary.apply(this, [
		itemIndex,
		imageFieldName,
		options.resizeFactor ?? 100,
	...
	const processImage = async ({ data: image, name, mimetype }: ImageWithName) => {
		...
		if (d === TIMEOUT) {
			await terminateTimedOutWorker.call(this, worker, name);
		}

		this.logger.debug('Image processed', { name });

		newItem.json =
			d === TIMEOUT ? { timeout: true } : { text: d.data.text, confidence: d.data.confidence };
		newItem.binary!['ocr'] = await this.helpers.prepareBinaryData(image, name, mimetype);
		return newItem;
	};

	const imagesData: INodeExecutionData[] = [];

	for (const image of images) {
		const imageData = await processImage(image);
		imagesData.push(imageData);

		// A timed-out worker has been terminated and cannot process
		// any remaining images from this PDF.
		if (imageData.json.timeout === true) {
			break;
		}
	}
	...
	return imagesData.filter(
		(imageData) =>
			((imageData.json.confidence as number) ?? options.minConfidence) >= options.minConfidence,
	);
}
...
export async function extractBoxes(
	...
	itemIndex: number,
	imageFieldName: string,
	granularity: 'paragraphs' | 'lines' | 'words' | 'symbols',
	options: OCROptions,
): Promise<INodeExecutionData[]> {
	const images = await getImagesFromBinary.apply(this, [
		itemIndex,
	...
	const processImage = async ({ data: image, name, mimetype }: ImageWithName) => {
		...
		if (d === TIMEOUT) {
			...
			// @ts-ignore
			d.data.lines = d.data
				.blocks!.map((block) => block.paragraphs.map((paragraph) => paragraph.lines))
				.flat(2);
			// @ts-ignore
			d.data.words = d.data
				.blocks!.map((block) =>
					block.paragraphs.map((paragraph) => paragraph.lines.map((line) => line.words)),
				)
				.flat(3);
			// @ts-ignore
			d.data.symbols = d.data
				.blocks!.map((block) =>
					block.paragraphs.map((paragraph) =>
						paragraph.lines.map((line) => line.words.map((word) => word.symbols)),
					),
				)
				.flat(4);
