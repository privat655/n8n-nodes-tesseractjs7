...
async function timedRender(renderer: IsolatedPdfRenderer, page: number, dpi: number, timeout: number): Promise<Buffer> {
	let timer: ReturnType<typeof setTimeout> | undefined;
	try {
		return await Promise.race([
			renderer.render(page, dpi),
			new Promise<never>((_, reject) => {
				timer = setTimeout(() => reject(new Error(`Rendering page ${page} exceeded ${timeout} ms`)), timeout);
			}),
		]);
	} finally {
...
export class PdfPagesToPng implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'PDF Pages to PNG',
		name: 'pdfPagesToPng',
		icon: 'file:pdfPagesToPng.svg',
		group: ['transform'],
		version: 1,
		...
		properties: [
			...
			{ displayName: 'DPI', name: 'dpi', type: 'number', default: 150, typeOptions: { minValue: 36, maxValue: 600 }, description: 'Render resolution; large drawings may require a lower value to fit the pixel limit' },
			{
				...
				options: [
					...
					{ displayName: 'Render Timeout (Ms)', name: 'renderTimeoutMs', type: 'number', default: 60000, typeOptions: { minValue: 1, maxValue: 300000, numberPrecision: 0 }, description: 'Timeout for rendering one page after the renderer has started' },
				],
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			try {
				...
				// No OCR worker, no global canvas patching, and only one live raster at a time.
				...
				for (const page of plan) {
					const png = await timedRender(renderer, page.page, dpi, renderTimeout);
				...
				if (renderer) await renderer.terminate();
			}
		}
		return [output];
	}
