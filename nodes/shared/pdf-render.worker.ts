...
async function start(): Promise<void> {
	if (!parentPort) throw new Error('PDF renderer worker requires a parent port');
	...
	parentPort.on('message', async (message: WorkerInput) => {
		...
		try {
			...
			entry = pdf.canvasFactory.create(Math.ceil(viewport.width), Math.ceil(viewport.height));
			if (!entry.canvas || !entry.context) throw new Error('PDF.js did not create a canvas');
			await page.render({
				canvasContext: entry.context,
				viewport,
				background: '#ffffff',
			}).promise;
			const png = Uint8Array.from(entry.canvas.toBuffer('image/png'));
